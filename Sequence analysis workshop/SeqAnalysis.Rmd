...
```{r }
family <- read_dta(paste0(getwd(),"/","PartnerBirthbio.dta"))
...
write.csv(ob1,file = paste0(getwd(),"/","mytb.csv"))

# proportions of time spent in each state
seqmeant(family_month_seq, prop = TRUE)

# sequence of distinct successive states = number of episodes
...
tp <- seqistatd(family_month_seq) #returns a matrix col = each state; row = individual; cell = # of time points in each state
family <- tibble(family,as_tibble(tp))
```
...
getwd()
family$MARc2orplus<- family$`MARc2+` # name MARc2+ is illegal for write_dta
family$`MARc2+` <- NULL #delete a column
write_dta(family, path = paste0(getwd(),"/","family_cl.dta"))
```
